...
void SimplexWidget::setConstraints(const ca::vec_view<double>& constr) { 
...
void SimplexWidget::setFunction(const ca::vec_view<double>& constr) { 
...
void SimplexWidget::setVariables(const ca::mat_view<double>& vars )  { 
	const QSignalBlocker blocker_r(m_rows);
	const QSignalBlocker blocker_c(m_cols);
...
const ca::mat_view<double> SimplexWidget::variables() const { 
...
const ca::vec_view<double> SimplexWidget::constraints() const { 
	return m_variables->matrix().col(m_variables->matrix().cols()-1, 1);
};

const ca::vec_view<double> SimplexWidget::function() const { 
