#ifndef _CALGO_OPT_TESTING_DRAW_HPP_
#define _CALGO_OPT_TESTING_DRAW_HPP_
...
public:
	...
	template<typename T>
	void box(const ca::opt::Size2D<T>& size, std::size_t x, std::size_t y) {
		if (size == ca::opt::Size2D<T>{0, 0})
			return;
		point(x, y, '+');
		if (size.width() > 1) {
	...
	template<typename T>
	void draw(const ca::opt::Box2D<T>& ibox) {
		box(
			ibox.size(), 
			ibox.x(), ibox.y()
		);
...
template<typename T>
void drawBoxesRow(const std::vector<ca::opt::Box2D<T>*>& boxes) {
	T maxHeight = (*std::max_element(
		boxes.cbegin(), boxes.cend(), 
		[](const ca::opt::Box2D<T>* a, const ca::opt::Box2D<T>* b) {
	...
	T totalWidth = std::accumulate(
		...
		[](T sum, const ca::opt::Box2D<T>* box) {
...
template<typename T>
void drawBoxesPlane(const std::vector<ca::opt::Box2D<T>*>& boxes, const ca::opt::Packing2D<T>& pack) {
