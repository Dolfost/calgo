...
static const std::vector<std::vector<ca::opt::Box2D<int>>> boxSet = {
	{
		{5, 8}, {7, 3}, {7, 15}, {9, 4}, {8,10}, {2, 5}, {8, 12},
	}, {
		{6, 3}, {15, 4}, {10, 6}, {8, 3}, {11, 7}, {5, 12}, {7, 10},
	}, {
		{8, 1}, {5, 4}, {11, 7}, {4, 2}, {9, 3}, {2, 7}, {7, 7},
	}
};

template<typename T>
std::vector<ca::opt::Box2D<T>*> binaryBs(T p) {
	std::vector<ca::opt::Box2D<T>*> bs;
	for (int i = 0; i < std::log2(p); ++i) {
		T side = std::pow(2, std::log2(p) - i);
		int num_boxes = std::pow(2, i);
		for (int j = 0; j < num_boxes; ++j)
			bs.push_back(new ca::opt::Box2D<T>(side, side));
	}	
